...
export default function KanbasNavigation() {
    ...
    return (
        <div id="wd-kanbas-navigation" className="list-group rounded-0">
            ...
            <Link key="/Kanbas/Account" to="Kanbas/Account" className={`list-group-item border-0 text-center ${pathname.includes("Account") ? "bg-white text-danger" : "bg-black text-white"}`}>
            ...
            {links.map((link) => 
                (
                    <Link key={link.path} to={link.path} className={`list-group-item border-0 text-center ${pathname.includes(link.label) ? "bg-white text-danger" : "bg-black text-white"}`}>
